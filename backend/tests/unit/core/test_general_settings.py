...
def test_build_effective_settings_ignores_non_runtime_editable_keys() -> None:
    resolved_settings = build_effective_settings(
        [
            {
                "key": "DATABASE_URL",
                "db_value": "postgresql+psycopg://postgres:postgres@localhost/override",
                "use_db_value": True,
                "is_runtime_editable": True,
            }
        ]
    ...
    assert resolved_settings.DATABASE_URL != "postgresql+psycopg://postgres:postgres@localhost/override"
